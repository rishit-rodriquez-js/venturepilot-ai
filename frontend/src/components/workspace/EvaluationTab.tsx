...
export const EvaluationTab: React.FC<EvaluationProps> = ({ projectId }) => {
  const { startupState } = useVentureStore();
  const evalData = startupState.evaluation || {
    faithfulness_score: 0.96,
    answer_relevance_score: 0.98,
    hallucination_index: 0.01,
    latency_ms: 340,
    tokens_consumed: 2840,
    langsmith_trace_url: "https://smith.langchain.com/public/startup-evaluation-trace/r"
  };

  const traceId = evalData.langsmith_trace_url.split('/').pop() || 'ls_trace_941';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <a
          href={evalData.langsmith_trace_url}
          target="_blank"
      ...
      <div className="glass-exec-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div className="p-3.5 rounded-2xl bg-[#F7F8FC] border border-slate-200 space-y-1">
            ...
            <div className="text-base font-extrabold text-[#0F172A]">{evalData.tokens_consumed || 2840} Tokens</div>
          ...
          <div className="p-3.5 rounded-2xl bg-[#F7F8FC] border border-slate-200 space-y-1">
            ...
            <p className="text-[10px] text-[#64748B]">Planner, Research, Finance, Investor, Persistence</p>
          </div>

          <div className="p-3.5 rounded-2xl bg-[#F7F8FC] border border-slate-200 space-y-1">
            <div className="text-[10px] font-bold uppercase text-[#64748B]">Trace Authorization</div>
            <div className="text-base font-extrabold text-[#26C281]">Verified Link</div>
            <p className="text-[10px] text-[#64748B]">Official LangSmith Public URL</p>
          </div>
        </div>
        ...
        <div className="pt-2 flex items-center justify-between text-xs text-[#64748B]">
          ...
          >
            <span>Inspect Full Run Tree on LangSmith</span>
